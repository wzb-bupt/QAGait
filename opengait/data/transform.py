...
class RandomPerspectiveFan(object):
    def __init__(self, prob=0.5):
        self.prob = prob
...
class RandomRotate(object):
    def __init__(self, prob=0.5, degree=20, lean=False):
        self.prob = prob
        self.degree = degree
        self.lean = lean
...
class AlignRandomDisturb(object):
    def __init__(self, prob=1.0, disturb=5, per_frame=False):
        self.prob = prob
    ...
    def __call__(self, seq):
        if random.uniform(0, 1) >= self.prob:
            ...
            if not self.per_frame:
                _, dh, dw = seq.shape
                # rotation
                disturb = random.uniform(-self.disturb, self.disturb)
                M1 = cv2.getRotationMatrix2D((dh // 2, dw // 2), disturb, 1)
                # affine
                seq = [cv2.warpAffine(_[0, ...], M1, (dw, dh))
                    for _ in np.split(seq, seq.shape[0], axis=0)]
                seq = np.concatenate([np.array(_)[np.newaxis, ...]
                ...
                _, dh, dw = seq.shape
                # affine
                seq = [cv2.warpAffine(_[0, ...], cv2.getRotationMatrix2D((dh // 2, dw // 2), random.uniform(-self.disturb, self.disturb), 1), (dw, dh))
                    for _ in np.split(seq, seq.shape[0], axis=0)]
                seq = np.concatenate([np.array(_)[np.newaxis, ...]
                                    for _ in seq], 0)
            return seq
